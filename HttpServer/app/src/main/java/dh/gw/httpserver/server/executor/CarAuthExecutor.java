...
public class CarAuthExecutor implements IExecutor {

    private static final String TAG = CarAuthExecutor.class.getSimpleName();

    @Override
    public Result asyncExecute(IJsonReader json) {

        String token = json.getSubJson(URLKey.BODY).getValue(URLKey.TOKEN);
        String carFrameNum = json.getSubJson(URLKey.BODY).getValue(URLKey.CAR_FRAME_NUM);
        String plateNumber = json.getSubJson(URLKey.BODY).getValue(URLKey.PLATE_NUM);

        ILog.d(TAG,"[checkCarNmu]--"+"["+token+","+carFrameNum+","+plateNumber+"]");
